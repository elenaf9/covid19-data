...
export class PopulationService {
  ...
  async createOrFindPopulationData(
    population: Population,
  ): Promise<Population> {
    const existingPopulationData: Population[] = await this.populationModel
      .find({ countryGeoId: population.countryGeoId, year: population.year })
      .exec();
  ...
  async getPopulationDataValue(geoId: string): Promise<number> {
    const existingPopulationData: Population[] = await this.populationModel
      .find({ countryGeoId: geoId })
      .exec();
    if (existingPopulationData.length) {
      return existingPopulationData[0].population;
    }
    return 0;
  }
}
